"""
XAI Forensic Terminal — Flask Backend (V4 A/B Test Sandbox)
Reads dynamic JSONL files from ../logs/ and serves alerts via REST API.
Run: python app_v4.py  (from inside the dashboard/ directory)
"""
...
def get_available_cases() -> list[str]:
    ...
    # [V4 SANDBOX CHANGE]: Only find jsonl files that end in "v4" so we don't mix V1 and V4 logs!
    files = list(LOGS_DIR.glob("*v4.jsonl"))
...
def index():
    ...
    # [V4 SANDBOX CHANGE]: Serve the V4 dashboard html
...
if __name__ == "__main__":
    print(f"[*] XAI Forensic Terminal (V4 Sandbox) starting...")
    print(f"[*] Scanning for audit logs in: {LOGS_DIR.resolve()}")
    print(f"[*] Found {len(get_available_cases())} V4 log file(s).")
    
    # [V4 SANDBOX CHANGE]: Run on port 5001 to avoid crashing your V1 system
